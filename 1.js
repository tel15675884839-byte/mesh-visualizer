...
console.log('🚑 Applying V25: Fixing ReferenceError (Restoring States & Cleaning Dead Code)...');
...
// Removed 'dragOverride' prop as we use Imperative updates now
...
// --- SingleDeviceNode (V24+ logic with SVG) ---
const SingleDeviceNode = React.memo(({ 
    ...
    // V26: Increase Leader/Router Size
  ...
  return (
    <Group>
      {activeFloor.nodes.map((node: any) => (
         <SingleDeviceNode 
            key={node.id} // Or key={\`\${node.id}-\${node.category}\`} if we want aggressive remounting
            node={node}
...
export const FloorPlanEditor = () => {
  ...
  // RESTORED: Reference to track drag target physically (fixes Sticky Drag)
  ...
  // RESTORED: State to lock stage dragging when node is moving
  ...
  useEffect(() => {
      const handleFocusEvent = (e: any) => {
          ...
          // Set Highlight
          if (e.detail.id) { setHighlightedId(e.detail.id); setTimeout(() => setHighlightedId(null), 3000); }
  ...
  const handleMouseDown = (e: any) => {
    // If clicking on stage, reset everything
  ...
  const handleOpenProperties = () => {
      // Logic handled in render directly now
  };

  // Node Drag Callbacks (Stable)
        ...
        {!activeFloor ? <div className="flex items-center justify-center h-full text-gray-400 text-sm">Select a floor to start editing</div> : (
          <>
            <Stage 
                ...
                draggable={!isDraggingNode && !isDeleteMode} // Only stage draggable if not dragging node
                onWheel={handleWheel} 
                ...
                onDragEnd={(e) => { 
                    // Store View State ONLY if stage was dragged
            ...
            >
                ...
                <Nodes 
                    ...
                    onDragStart={handleDragStartNode} // Use Callback
                    onDragEnd={handleDragEndNode}     // Use Callback
                />
...
console.log('✅ FloorPlanEditor.tsx: Sticky Drag & Icons Fixed (V25 Final).');